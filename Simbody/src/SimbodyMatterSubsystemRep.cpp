...
// EU Realize velocities from modal velocities
int SimbodyMatterSubsystemRep::realizeSubsystemModalVelocityImpl(const State& s, SimTK::Vector& ModalV) const {
    SimTK_STAGECHECK_GE_ALWAYS(getStage(s), Stage(Stage::Velocity).prev(), 
        "SimbodyMatterSubsystem::realizeVelocity()");
    ...
    // EU Additional quantities
    // abc needed for realizeModalVelocity
    const SBArticulatedBodyInertiaCache& abc = getArticulatedBodyInertiaCache(s);

    // Set generalized speeds: sweep from base to tips.
    for (int i=0 ; i<(int)rbNodeLevels.size() ; ++i) 
        for (int j=0 ; j<(int)rbNodeLevels[i].size() ; ++j)
            rbNodeLevels[i][j]->realizeModalVelocity(abc, stateDigest, ModalV); 
    ...
    Vector&                                                 MInvf) const 
{
    ...
    for (int i=0 ; i<(int)rbNodeLevels.size() ; i++)
        for (int j=0 ; j<(int)rbNodeLevels[i].size() ; j++) {
            //std::cout<<"multiplyByMInv rbNodeLevel ("<<i<<','<<j<<')'<<std::endl; // EU
            const RigidBodyNode& node = *rbNodeLevels[i][j];
    ...
    Vector&                                                 MInvf) const 
{
    ...
    for (int i=rbNodeLevels.size()-1 ; i>=0 ; i--){
        for (int j=0 ; j<(int)rbNodeLevels[i].size() ; j++) {
            std::cout<<"multiplyBySqrtMInv inward node "<<i<<" "<<j<<std::endl;
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.multiplyBySqrtMInvPass1Inward(ic,tpc,abc,dc,
                fPtr, z.begin(), zPlus.begin(), eps.begin());
        }
    }
    */
...
//............................. EU CALC SQRT M INVERSE F  EU ...............................

//==============================================================================
//                            EU CALC DET M INV EU
//==============================================================================
// Calculate udot = M^-1 f. We also get spatial accelerations A_GB for 
// each body as a side effect.
// This Subsystem must already be realized through Dynamics stage.
...
// must already have been realized to Position stage.
// It is OK if MInv's data is not contiguous.
void SimbodyMatterSubsystemRep::calcSqrtMInv(const State& s, Matrix& MInv) const {
    const int nu = getTotalDOF();
    MInv.resize(nu,nu);
    if (nu==0) return;
