...
//#include <Eigen/Dense> // EU
//#include <unsupported/Eigen/MatrixFunctions> // EU
...
// EU Pass 2 of multiplyBySqrtMInv.
// Base to tip: temp allA_GB does not need to be initialized before
// beginning the iteration.
template<int dof, bool noR_FM, bool noX_MB, bool noR_PF> void 
...
/*
    ...
    Mat<dof,dof> Z(1);
    Mat<dof,dof> sqrtDI(0);
    sqrtDI = DI;
    for(int i=0; i<5; i++){
      sqrtDI = 0.5*(sqrtDI + (Z.invert()));
      Z = 0.5*(Z + (sqrtDI.invert()));
    }
*/
/*
    ...
    //std::cout<<"RigidBodyNodeSpec Outward sqrtDI:"<<sqrtDI<<std::endl;
*/
